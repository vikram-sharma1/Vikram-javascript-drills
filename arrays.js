...
let getCarDetails = 33

for(let i = 0; i < inventory.length; i++){
    if(inventory[i].id == getCarDetails){
...
for(let i = 0; i < inventory.length; i++){
    if(inventory[i].id == inventory.length){
        console.log(`Last Car is a ${inventory[i].car_year} ${inventory[i].car_make} ${inventory[i].car_model}`)
    }
}
...
// Tried Iterative Method using some help from google


function iterativeSort(a,b){
...
// console.log(inventory.sort((a,b)=>iterativeSort(a.car_model, b.car_model)))

    // Using Inbuilt Function --------------

    let ans = inventory.sort((a,b)=> a.car_model.localeCompare(b.car_model))
        console.log(ans)
...
console.log("Years of all cars",carYears)
